...
impl GameEngine for SimpleGameEngine {
    ...
    fn register_event(&self, step_index: Option<u64>, event: EngineEvent) {
        ...
        self.pending_engine_events.borrow_mut().push((step_index, event));
    }

    fn spawn_entity(&self, entity: EngineEntity, step_index: Option<u64>, is_non_determ: bool) {
        self.register_event(step_index, EngineEvent::SpawnEntity { entity, universal: is_non_determ });
    }

    fn remove_entity(&self, id: &u128, step_index: Option<u64>, is_non_determ: bool) {
        self.register_event(step_index, EngineEvent::RemoveEntity { entity_id: *id, universal: is_non_determ });
    }
    ...
    fn step(&mut self) -> Vec<GameEvent> {
        ...
        for (step_index, event) in self.pending_engine_events.borrow_mut().drain(..).collect::<Vec<_>>() {
            if step_index != self.step_index {
                self.pending_engine_events.borrow_mut().push((step_index, event));
                continue;
            }
            match event {
                EngineEvent::SpawnEntity { entity, universal: _ } => {
