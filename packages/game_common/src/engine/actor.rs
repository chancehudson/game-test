...
pub fn contains_platform(engine: &GameEngine<KeindGameLogic>, rect: IRect) -> bool {
...
pub fn on_platform(body: IRect, engine: &GameEngine<KeindGameLogic>) -> bool {
...
pub fn can_move_left_right(body: IRect, engine: &GameEngine<KeindGameLogic>) -> (bool, bool) {
    (
        body.min.x > 2,
...
pub fn can_move_left_right_without_falling(
    body: IRect,
...
pub fn move_x<G: GameLogic>(body: IRect, dx: i32, engine: &GameEngine<G>) -> i32 {
    if dx == 0 {
        return body.min.x;
    }
...
pub fn move_y<G: GameLogic, T: SEEntity<G>>(
    body: IRect,
    dy: i32,
    platforms: &[&T],
    map_size: &IVec2,
